...
public class FileUploadHandlerBase extends IFileUploadHandlerPostImpl {
    ...
    public FileUploadHandlerBase(FilehandlerLog fhl, MultipartFile file, FileHandlerBufferRepository fileHandlerBufferRepository, FileHandlerLogRepository fileHandlerLogRepository, FileHandlerDetailLogRepository fileHandlerDetailLogRepository, FileHandlerAtomLogRepository fileHandlerAtomLogRepository,
        ...
        super(FileTypeEnum.BASE, fhl,file, fileHandlerBufferRepository, fileHandlerLogRepository, fileHandlerDetailLogRepository,fileHandlerAtomLogRepository,
...
//                filetype.equalsIgnoreCase("application/vnd.ms-excel");
    ...
    public void validate() throws UnsupportedFileFormatException {
        ...
        if (!validatefileformat(fhlb.getFhlb_Body())) throw new UnsupportedFileFormatException(fhlb.getFhlb_Name(),FileTypeEnum.BASE);;
    }
    ...
    protected boolean process(Row row) throws JsonProcessingException {
        ...
        try {
            ...
            declaration_cache.dc_scountry = scountry_iso2; //по умолчанию
            declaration_cache.dc_rcountry = rcountry_iso2; //по умолчанию
            declaration_cache.dc_service_id = service_id.toString();
            declaration_cache.dc_type_id = type_id.toString();
            declaration_cache.dc_createdate = new Date(System.currentTimeMillis());
...
//                declaration_cache.dc_rstype=formatter.formatCellValue(row.getCell(19));
            ...
            if ((row.getCell(26)!=null) && !isCellEmpty(row.getCell(26)))
                declaration_cache.dc_scountry = formatter.formatCellValue(row.getCell(26));
            else declaration_cache.dc_scountry = scountry_iso2;

            if ((row.getCell(27)!=null) && !isCellEmpty(row.getCell(27)))
                declaration_cache.dc_rcountry = formatter.formatCellValue(row.getCell(27));
            else declaration_cache.dc_rcountry = rcountry_iso2;
