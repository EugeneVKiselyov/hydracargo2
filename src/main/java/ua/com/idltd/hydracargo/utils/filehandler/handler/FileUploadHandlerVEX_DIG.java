...
public class FileUploadHandlerVEX_DIG extends IFileUploadHandlerPostImpl {
    ...
    public FileUploadHandlerVEX_DIG(FilehandlerLog fhl, MultipartFile file, FileHandlerBufferRepository fileHandlerBufferRepository, FileHandlerLogRepository fileHandlerLogRepository, FileHandlerDetailLogRepository fileHandlerDetailLogRepository, FileHandlerAtomLogRepository fileHandlerAtomLogRepository,
...
//                filetype.equalsIgnoreCase("application/vnd.ms-excel");
    ...
    public void validate() throws UnsupportedFileFormatException {
        ...
        if (!validatefileformat(fhlb.getFhlb_Body())) throw new UnsupportedFileFormatException(fhlb.getFhlb_Name(),FileTypeEnum.VEX_DIG);;
    }
    ...
    protected boolean process(Row row) throws JsonProcessingException {
        ...
        try {
            ...
            int tmp;
            try {
                tmp = new Double(row.getCell(0).getNumericCellValue()).intValue();
            } catch (Exception e){
                tmp = new Double(row.getCell(0).getStringCellValue()).intValue();
            }
            shipment_tmp=String.format("GE%010d"+entrepot.ep_code+"-UA",tmp);

            declaration_cache.dc_shipment = shipment_tmp;
